...
function F_loginForm($faction, $fid, $fmethod, $fenctype, $username, $password, $isize=20) {
	...
	if (K_USRREG_ENABLED) {
		$str .= '<small><a href="tce_user_registration.php" title="'.$l['t_user_registration'].'">'.$l['w_user_registration_link'].'</a></small>'.K_NEWLINE;
	}
	$str .= '<div class="tceformbox">'.K_NEWLINE;
	$str .= '<form action="'.$faction.'" method="'.$fmethod.'" id="'.$fid.'" enctype="'.$fenctype.'">'.K_NEWLINE;
	
	//$str .= '<fieldset style="border:none;margin:0;padding:0;">'.K_NEWLINE;
