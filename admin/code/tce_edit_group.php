...
if (isset($group_id) AND ($group_id > 0)) {
	// display users that belongs this group
	echo '<fieldset class="subset" style="text-align:left;">'.K_NEWLINE;
	echo '<legend>'.$l['w_users'].'</legend>'.K_NEWLINE;

	$sql = 'SELECT user_id, user_lastname, user_firstname, user_name, user_email
	...
	if ($r = F_db_query($sql, $db)) {
		echo '<ul>'.K_NEWLINE;
		while ($m = F_db_fetch_array($r)) {
			echo '<li><a href="tce_edit_user.php?user_id='.$m['user_id'].'" title="'.$l['t_user_editor'].'">'.htmlspecialchars($m['user_lastname'].' '.$m['user_firstname'].' ['.$m['user_name'].'] - '.$m['user_email'], ENT_NOQUOTES, $l['a_meta_charset']).'</a></li>'.K_NEWLINE;
		}
		echo '</ul>'.K_NEWLINE;
	} else {
		F_display_db_error();
