...
if (isset($_REQUEST['user_sshkey']) AND !empty($_REQUEST['user_sshkey'])) {
	$user_sshkey = $_REQUEST['user_sshkey'];
	if (preg_match('/(ssh\-rsa|ssh\-dss)[\s]+([^\s]+)/', $user_sshkey, $matches) > 0) {
		$user_sshkey = $matches[1].' '.$matches[2];
	} else {
		$user_sshkey = '';
	}
} else {
...
if (isset($_REQUEST['user_sshkey'])) {
	$user_sshkey = htmlspecialchars($_REQUEST['user_sshkey'], ENT_COMPAT, $l['a_meta_charset']);
} else {
	$user_sshkey = '';
}
