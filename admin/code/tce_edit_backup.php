...
/**
 * @file
 * Backup and Restore Database (works only on POSIX systems with MySQL).
...
if (isset($backup_file) AND !empty($backup_file)) {
	if ((preg_match('/[^a-zA-Z0-9\_\-\.]+/i', $backup_file) > 0) OR (strlen($backup_file) != 36) OR (substr($backup_file, -3) != '.gz')) {
		// ERROR
		...
		if($forcerestore == $l['w_restore']) { //check if delete button has been pushed (redundant check)
			if (isset($backup_file) AND !empty($backup_file)) {
				...
				switch (K_DATABASE_TYPE) {
					case 'POSTGRESQL': {
						$filename = K_PATH_BACKUP.date('YmdHis').'_rackmap_backup.tar';
					...
					case 'MYSQL':
					default: {
						$filename = K_PATH_BACKUP.date('YmdHis').'_rackmap_backup.sql';
				...
				exec($command);
				// substring file name for security reason
				$backup_file = substr($backup_file, 0, 36);
